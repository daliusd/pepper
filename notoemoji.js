...
const PATH_TO_SEARCH = '../noto-emoji/svg/';
...
for (const emoji of emojiJson) {
    ...
    if (emoji.non_qualified) {
        const code = emoji.non_qualified.toLowerCase();
        emojiMap[code] = name;
    }

    if (emoji.unified) {
        const code = emoji.unified.toLowerCase();
        emojiMap[code] = name;
    }
    ...
    for (const svKey in emoji.skin_variations) {
        ...
        if (sv.non_qualified) {
            const code = sv.non_qualified.toLowerCase();
            emojiMap[code] = name + '_' + svKey;
        }

        if (sv.unified) {
            const code = sv.unified.toLowerCase();
            emojiMap[code] = name + '_' + svKey;
        }
    }
}
    ...
    for (const image of svgs) {
        ...
        if (!(name in emojiMap)) {
            ...
            try {
                const formData = {
                    global: 'true',
                    name: `noto_${emojiName}`,
                    metadata: JSON.stringify({ source: 'noto-emoji' }),
                    image: {
                        value: svg_data,
                        options: {
                            filename: name + '.svg',
